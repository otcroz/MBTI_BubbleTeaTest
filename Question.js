...
//1. 배경
//배경 순차적으로 출력
QueBackground = ['url("tea_bg/버블티_1.png")', 'url("tea_bg/버블티_2.png")', 'url("tea_bg/버블티_3.png")', 'url("tea_bg/버블티_4.png")',
 'url("tea_bg/버블티_5.png")', 'url("tea_bg/버블티_6.png")', 'url("tea_bg/버블티_7.png")', 'url("tea_bg/버블티_8.png")',
  'url("tea_bg/버블티_9.png")', 'url("tea_bg/버블티_10.png")', 'url("tea_bg/버블티_11.png")'];

//2. 질문창
//질문 내용 리스트
QuestionList = [
    "버블티 가게에서 새로운 신상 버블티가 나왔다고 하네요! 새로 나온 버블티라 맛이 궁금한데.. 당신은 어떻게 할 건가요?",
...
btn1ContextList = [ //E, S, F, P 지수
    "맛이 어떨지 궁금하니까 버블티 가게에서 새로 나온 버블티를 먹고 어떤 맛인지 알고 싶어.",
...
btn2ContextList = [ //I, N, T, J 지수
    "맛은 궁금하지만 일단 새로 나온 버블티의 후기를 찾아볼래!",
    "집에서 누워있으면서 쉬고 싶어",
...
function init1(){
    ...
    b1.addEventListener('click',mbtiElement);
    b1.addEventListener('click',bCilck);
    b1.addEventListener('click',clickBtn);
    
    b2.addEventListener('click',mbtiElement);
    b2.addEventListener('click',bCilck);
    b2.addEventListener('click',clickBtn);
}

//mbti 각 지수 증가
function mbtiElement(){
    if (mbtiClickValue<3){
        if(event.target == b1 || event.target == sel1Context){
            e+=1;
        }
        else if (event.target == b2 || event.target == sel2Context){
            i+=1;
        }
    }
    else if (mbtiClickValue < 6){
        if(event.target == b1 || event.target == sel1Context){
            s+=1;
        }
        else if (event.target == b2 || event.target == sel2Context){
...
function clickBtn(){
    ...
    if (clickValue<12){
        ...
        window.location.href = "ResultPage.html?"+mbti;
    }
